...
The Penn Treebank dataset has 1,036,580 words, which is already split\footnote{https://github.com/wojzaremba/lstm} into a training set of 887,521 words, a validation set of 70,390 words, and a test set of 78,669 words. The size of the vocabulary is 10,000. We use the library gensim\footnote{https://radimrehurek.com/gensim/models/word2vec.html} to compute a word2vec model for embedding the words of the dictionary in a 200-dimensional space. Then we build the adjacency matrix of the word embeddings using the 4-nearest neighbor graph with cosine distance. Figure \ref{fig2} presents the computed adjacency matrix. The size of the batch is 20, the number of temporal steps to unroll is 21, the dimension of the hidden state is 200. The global learning rate is 0.02 for vanilla RNN and graphCNN variants, 0.05 for LSTM and graphCNN variants, and 0.1 for GRU. The learning decay function is the global learning rate times the $0.5^{\max(0,\textrm{\#epoch}-4)}$. All experiments have 8 epochs.


\begin{figure}[ht]
	\centering
	\includegraphics[width=6cm]{figures/matrixW.eps}
	\caption{Adjacency Matrix of Word Embeddings} 
	\label{fig2}
\end{figure}
...
Table \ref{tab1} reports the final perplexity values for each model and Figures \ref{tab3}
