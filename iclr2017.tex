...
minimizing the cross-entropy loss using back-propagation through time (BPTT)
and RMSProp with a learning rate of $10^{-3}$ and a decay rate of 0.9. All
implementations are based on their Theano code and
dataset.\footnote{\url{http://www.wanghao.in/code/SPARNN-release.zip}} The
graph is constructed by connecting each pixel with their four closest
neighbors.
...
\begin{table}
	...
	\begin{tabular}{lcccc}
		...
		LSTM+CNN \citep{convlstm} & $5 \times 5$ & 13,524,496 & 2.1 & 3841 \\
		LSTM+GCNN & $K=3$ & 1,629,712 & 0.82 & 4132\\
		LSTM+GCNN & $K=5$ & 2,711,056 & 1.53 & 3521\\
		LSTM+GCNN & $K=7$ & 3,792,400 & 1.75 & 3445\\
		\bottomrule
	\end{tabular}
	\caption{Comparison between models. Cross-entropy of FC-LSTM is taken from \cite{convlstm}. LSTM-GCNN is Model 2 defined in \eqnref{lstm_graph_v2}.}
...
\begin{figure}[ht]
	...
	\caption{An example showing a run. First row is the input sequence, second the ground truth and third the predictions of the 1-layer LSTM+GCNN with $K=5$.}
\end{figure}
