...
\section{Experiments}
\label{experiments}
Proposing Graph Convolutional Recurrent Neural Networks(GCRNN) applies two benchmark datasets: Penn Tree Bank\citet{ptb} and moving MNIST\citet{moving_mnist}. Experiments are mainly focuses on how the graph convolution give effects on RNN model by comparing conventional RNN models with GCRNN. In moving MNIST experiments, we shows 2D grid structure is one of special case of graph structure which GCRNN can easily fit to the model as convolutional RNN.
...
Following the experimental setup of \citet{convlstm} Each of sample in moving MNIST data has 20 number of sequential images containing 2 randomly selected handwritten digit from MNIST dataset. In each frame, two digits are moving inside of 64 $\times$ 64 image by changing their positions. 


In addition, we add a rotational option to make each of digit rotates 360 degrees through time to show proposing GCRNN is robust to rotations of the image. 
...
ability of our model \eqnref{lstm_graph_v2} to learn spatio-temporal structures.
