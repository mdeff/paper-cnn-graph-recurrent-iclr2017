...
succeeding frames share temporal and spatial structures. \textcolor{red}{Great attentions on structured sequenctial data triggers novel approaches such as \citet{GGSNN} which combines Graph Neural Networks\citet{GNN} with recurrent neural network model.} This model uses sparse adjecency matrix which represents relationship between nodes in graph to feed graph-structure as a input of neural networks. On the other hands, Many works, such as \citet{cnnlstm1, cnnlstm2, cnnlstm3},  leveraged a combination of CNN and RNN to exploit such spatial and
