...
succeeding frames share temporal and spatial structures. \textcolor{red}{Great attentions on structured sequenctial data triggers novel approaches such as \citet{GGSNN} which combines Graph Neural Networks\citet{GNN} with recurrent neural network model. This model uses sparse adjecency matrix which represents relationship between nodes in graph to feed graph-structure as a input of neural networks. This inforces neural networks to learn a structural relationship between the nodes and to achieve higher performance than conventional neural network models without considering graph structure.} On the other hands, Many works, such as \citet{cnnlstm1, cnnlstm2, cnnlstm3},  leveraged a combination of CNN and RNN to exploit such spatial and
...
For this synthetic experiment, we use the publicly available
dataset\footnote{\url{http://www.wanghao.in/code/SPARNN-release.zip}} generated
by \citet{convlstm}. All sequences are 20 frames long (10 frames for the input
and 10 frames for the prediction) and contain two handwritten digits bouncing
inside a $64 \times 64$ patch. Following the experimental setup of
\citet{convlstm}, all the models are trained by minimizing the cross-entropy
loss using back-propagation through time (BPTT) and RMSProp with a learning
rate of $10^-3$ and a decay rate of 0.9.
...
\begin{table}
	...
	\begin{tabular}{lcccc}
		\toprule
		Architecture & Support & Parameters & Running time & Cross-entropy \\
		\midrule
		FC-LSTM \citep{convlstm} & N/A & 142,667,776 & N/A & 4832 \\
		LSTM+CNN \citep{convlstm} & $5 \times 5$ & 13,524,496 & 2.1 & 3982 \\
		LSTM+GCNN & $K=3$ & 1,629,712 & 0.82 & 4438\\
		LSTM+GCNN & $K=5$ & 2,711,056 & 1.53 & 3741\\
		LSTM+GCNN & $K=7$ & 3,792,400 & 1.75 & N/A \\
		\bottomrule
	\end{tabular}
...
\begin{figure}[ht]
	...
	\caption{Convergence of the various models.} 
\end{figure}

\tabref{moving_mnist} shows the performance of various models: (i) the baseline
FC-LSTM from \citet{convlstm} (performance measure from them), (ii) the 1-layer
LSTM+CNN from \citet{convlstm} and (iii) the proposed LSTM+GCNN defined by
\eqnref{lstm_graph_v2} with three different values of $K$. These results show
the ability of the proposed method to capture spatio-temporal structures.
...
\begin{figure}[h!]
	\centering
	\subfigure{\includegraphics[width=0.48\linewidth]{figures/train_ptb}}
	\hfill
	\subfigure{\includegraphics[width=0.48\linewidth]{figures/test_ptb}}
