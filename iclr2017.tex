...
nodes of the graph may be regarded as a matrix $x_t \in \R^{d_x \times n}$
...
principle $\bO({d_x}^K n^K)$, we sought to exploit the structure of the space
...
paper, we follow the FC-LSTM formulation of \citet{seq_graves}, that is:
...
essentially the classical FC-LSTM \eqnref{lstm_fc} where the multiplication by
dense matrices $W$ have been replaced by convolutions with kernels $W$:
...
\paragraph{Recurrent convolutional neural network (rCNN).} Similarly to
\citet{convlstm}, \citet{video_language_model} proposed a RNN variation which
...
be filters of size 1x1. Effectively a fully connected layer which is shared
