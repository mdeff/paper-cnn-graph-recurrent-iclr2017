...
\begin{abstract}
	...
	data structured by a weighted graph. So far the only underlying structures that have been explored in the sequence modeling literature are the linear chain, the 2D grid and the tree.
	...
	This work introduces a general framework for structured sequence modeling,
	studies two possible architectures and apply the model to two practical
	problems: a benchmark moving MNIST dataset and a language model on the Penn
	Treebank. Experiments \todo{show that our network}
\end{abstract}
...
In this paper, we are interested in structured sequences, i.e. sequences where
some dimensions of the observations $x_t$ are not independent but linked by
...
heterogeneous pairwise relationships. We can view $x_t$ as a graph signal, i.e.
...
principle $\bO(n^K {d_x}^K)$, we sought to exploit the structure of the space
...
introduced by \citet{lstm} and designed to prevent the gradient from vanishing
too quickly, has proven stable and powerful for modeling long-range
dependencies in various general-purpose sequence modeling tasks
...
recent area of interest. Two approaches are explored in the literature: (i) a
...
different meshes.  Hence, this method requires a manifold and is not
...
conventional 1D CNN. As graphs generally don't possess a natural ordering
...
described below in the context of graph CNNs. The major drawback of their
method is its $\bO(n^2)$ complexity. \citet{gcnn} essentially fixed that,
offering a linear complexity $\bO(|\E|)$ while providing strictly localized
...
classification of nodes. While we'll focus on the framework introduced by
...
\citet{convlstm} introduced a model for grid-structured sequences, which can be
seen as a special case of the proposed model where the graph is a grid and the
nodes are ordered. Their model is essentially the classical FC-LSTM
...
Remarking that natural language exhibits syntactic properties that naturally
...
\begin{figure}[ht]
	...
	%\framebox[4.0in]{$\;$}
	\fbox{\rule[-.5cm]{0cm}{5cm} \rule[-.5cm]{\linewidth}{0cm}}
%	\includegraphics[width=10cm]{figures/gcnn_rnn.eps}
...
network of $n$ sensors whose organization is given by the weighted graph $\G$.
The model holds spatially distributed hidden and cell states of size $d_h$
given by the matrices $c_t, h_t \in \R^{n \times d_h}$. Peepholes are
controlled by $w_{c\cdot} \in \R^{n \times d_h}$. The support $K$ of the graph
